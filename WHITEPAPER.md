...
*Whitepaper version 1.2 · status: `v0.2.x` published to npm under `@loomfsm/*` (interactive · headless · daemon · HTTP control plane); the `0.3.0` line — configure-once multi-backend execution, non-Claude work-agent harnesses, and a web dashboard with one-command `loom up` — is in development in the repo.*
