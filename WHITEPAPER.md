...
Scope honesty matters more than feature counting. The following are additive layers over the substrate — some now shipped in `0.3.0`, the rest planned for later:

- **Networked transport.** The local-process daemon (`loom daemon`) and an HTTP **control plane** (`loom serve` — submit / read-model / answer / SSE, multi-project supervision over loopback, with a reference chat-intake adapter on the same `/submit`) both ship. A **web dashboard** — a React SPA the server hosts at its bind address as prebuilt static assets (no runtime dependency added), a peer client of those same routes — **ships in `0.3.0`**; `loom up` (or a bare `loom`) brings the plane up on localhost and opens it. Still additive: richer task intake over the same routes — the same `drive()` behind an adapter, as stdio sits behind MCP. No kernel change.
- **Configure-once, any-model execution (shipped in `0.3.0`).** A control layer for global API keys + a per-agent model map + a browsable project catalog; per-spawn backend resolution (`auto` is Claude-Code-first when present, else a configured provider — OpenRouter / Ollama / Anthropic); and non-Claude **work-agent harnesses** — an agentic CLI (Aider, opencode) behind the same isolated-worktree `Executor` seam — so a *file-editing* agent runs fully off Claude while decision-agents stay a single model call. The harness is chosen by a generic, bundle-declared agent capability, never by name. Ships in `0.3.0` over the same `drive()` seam with **zero kernel change**; the executor + dispatch paths are validated against real non-Claude models, with hardening continuing. The same line surfaces this control layer in the web dashboard — config, secrets, and the per-agent model map edited through forms generated from the config schema, so the UI stays bundle-blind across rosters.
...
- **Validation**: the `code` bundle has driven real `/task` runs end to end through an MCP host to `complete:accepted`, with the audit log recording every spawn, finding, verdict, and gate. As of `0.2.0` the same flow also runs **headless** (`loom run`) and under the **daemon**, executing each spawn through `claude -p` in an isolated worktree; `0.2.1` adds a multi-project **HTTP control plane** (`loom serve`), optional **container-isolated** spawns, rate-limit-aware and timeout-bounded supervision, and opt-in **outbound notifications**. `0.3.0` adds the configure-once control layer, per-spawn multi-backend dispatch (validated against real non-Claude models), non-Claude work-agent harnesses, and a **web dashboard** with one-command `loom up`; `0.3.1` adds operator UX (a fast-task path, per-task Docker, live model lists, pause / resume / cancel, total elapsed, a human-readable log); `0.3.2` adds in-dashboard **observability** — the per-task agent-chain view (model, tokens, derived duration, findings / verdicts, the documents each run wrote) and an archived-task browser that reopens any finished task in the same view, both read straight from the per-task store with no kernel change. The deterministic paths are covered by the test suite, with an opt-in real-`claude` end-to-end.
...
*Whitepaper version 1.3 · status: `v0.3.2` published to npm under `@loomfsm/*` — interactive · headless · daemon · HTTP control plane · web dashboard (`loom up`) with per-task agent-chain observability + an archived-task browser · configure-once multi-backend execution · non-Claude work-agent harnesses.*
