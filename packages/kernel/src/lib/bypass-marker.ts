...
//   2. `~/.claude/bypass-hmac.key` — a user-global file (NOT project-
...
export interface LoadBypassKeyOptions {
  ...
  // `~/.claude/bypass-hmac.key`. It is NOT an env-controlled value — an
...
export function loadBypassKey(opts?: LoadBypassKeyOptions): BypassKey | null {
  ...
  const filePath = join(opts?.homeDir ?? homedir(), ".claude", KEY_FILE_NAME);
...
// `tx.now + ttl_ms`, parsing the NowToken string only. Mirrors the
// addMs/subtractMs helpers elsewhere — the `Date` use is on a supplied
// ISO-8601 string, never the host clock.
export function markerExpiresAt(now: NowToken, ttlMs: number): NowToken {
  const epoch = Date.parse(now);
  return new Date(epoch + ttlMs).toISOString() as NowToken; // allow-ambient-clock: parses the supplied NowToken string only; never reads the host clock
}
...
export async function issueCrossOwnerMarker(
  ...
  if (loaded === null) {
    throw new KernelError({
      ...
      message:
        "no bypass-HMAC key is configured — set PIPELINE_BYPASS_HMAC_KEY or install a user-global ~/.claude/bypass-hmac.key",
    });
  }
