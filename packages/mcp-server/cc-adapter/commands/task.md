...
- **`error`** with code **`PROJECT_TASK_ACTIVE`** → a task is already running in this project, so a new one was **not** started (and you must not start, rotate, or recover it on the user's behalf — they may have meant to continue the existing task or to start a genuinely different one). Show the active task — `mcp__loom__pipeline_state_get({ project_dir, format: "summary" })` — and offer three choices: **resume it** (`/resume`), **finish it** (`/done`), or **discard it and free the slot** (`loom reset`, or `loom reset --force` while it is still in progress). Let the user pick; do nothing else.
- **`error`** (any other code) → display `message` and `recovery_options` verbatim, then call `mcp__loom__pipeline_recover({ project_dir, driver_state_id, choice })` with the option the user picks (passing `agent_run_ids` when that option lists them).
