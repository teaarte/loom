...
//       eight) and calls every one of the eight, asserting the decoded
//       result shape. A test registry is injected so the active-task tools
//       have a flow to tick.
...
describe("wire smoke — functional (in-memory linked transport)", () => {
  it("tools/list returns exactly the eight registered tools", async () => {
